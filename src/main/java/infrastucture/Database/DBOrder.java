        ...
        try (Connection conn = Connector.getConnection()) {
            PreparedStatement ps =
                    conn.prepareStatement(
                            "INSERT INTO orders (userid, cupcakeid, price, paydate) " +
                                    "VALUE (?,?,?,?);",
                            Statement.RETURN_GENERATED_KEYS);
            ps.setInt(1, orderFactory.getUserId());
            ps.setString(2, orderFactory.getCupcakeId());
            ps.setDouble(3, orderFactory.getPrice());
